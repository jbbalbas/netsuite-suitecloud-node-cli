...
export default class CompilationContext {
	...
	constructor(options: any) {
		...
		this.extensions = extensions.map(
			(extension: any) => new Extension({ objectsPath: objectsPath, extensionXml: extension })
		);

		this.allExtensions = [this.theme].concat(this.extensions);
	}

	setLocalServerPath(path: string) {
	...
	getTemplates() {
		...
		this.allExtensions.forEach(
			(extension: any) => (templates = Object.assign(templates, extension.getTemplates()))
		);
		return this._handleOverrides(templates, this.getTplOverrides());
	}

	getSass() {
		const sass: {[x:string]: any} = {
			files: [],
			entrypoints: {},
		};

		this.allExtensions.forEach((extension: any) => {
	...
	getAssets() {
		let assets = {};
		this.allExtensions.forEach(
			(extension: any) => (assets = Object.assign(assets, extension.getAssets()))
		);
		return assets;
	}

	excludeBaseFilesPath(dir: string) {
		return path.relative(this.filesPath, dir);
	}

	_handleOverrides(resources: any, overrides: any) {
