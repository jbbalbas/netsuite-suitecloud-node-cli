...
export default class DeployXml {
	...
	initialize() {
		...
		objectsPath = path.join(this.projectFolder, objectsPath.substr(1));
		this.objectsPath = path.dirname(objectsPath);

		let filesPath = this.deploy.files.path;
		filesPath = path.join(this.projectFolder, filesPath.substr(1));
	...
	getObjects() {
		...
		const objectsPath: any[] = glob(path.join(this.objectsPath, '*'));
