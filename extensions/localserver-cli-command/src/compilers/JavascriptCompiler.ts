...
export default class JavascriptCompiler {
	...
	async _createFiles(resources: any) {
		/**
		...
		for (const resourcePath in resources) {
			...
			readFilesPromises.push(
				...
				resource.sourceContent().then((content: any) => {
					// then add the file content on each applicationFile
					resource.applications.forEach((appName: any) => {
	...
	_createModuleBlock(content: any, extName: any) {
		return `
	...
	_createTryCatchBlock(entrypoint: any, extName: any) {
		return `
		try {
